...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'vqav2_aug', 'vqav2_flip', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet'], 
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet"]:
            ...
            exit(0)
        elif args.task == "vqav2_aug":
            result, _ = evaluate(data_loader_test, model, device, task_handler)
            utils.dump_predictions(args, result, "vqav2_aug_test")
            exit(0)
        elif args.task == "vqav2_flip":
            result, _ = evaluate(data_loader_test, model, device, task_handler)
            utils.dump_predictions(args, result, "vqav2_flip_test")
            exit(0)
        elif args.task in ["coco_captioning", "nocaps"]:
